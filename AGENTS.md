...
- Project-detail root/main branch creation is an admin action in the page-level top actions, not inside the per-node branch column. The dialog should let the user choose the base root-node version before creating the new main/root branch.
- Workspace node responsibility assignment creates a new responsible branch. The dialog must ask for both branch name and assignee, and should not show the existing assigned-branch list.
- The main account is an administrator of every project by default. Project creation must include the main account as a locked admin member, and must also include the creator by default; project-member management must not allow the main account to be removed or downgraded.
